// Vercel Serverless Function: Anthropic API를 서버 쪽에서 안전하게 호출합니다.
// API 키는 클라이언트(브라우저)에 절대 노출되지 않고, Vercel 환경변수(ANTHROPIC_API_KEY)에서만 읽습니다.
...
module.exports = async (req, res) => {
  ...
  if (!apiKey) {
    res.status(500).json({ error: "서버에 ANTHROPIC_API_KEY 환경변수가 설정되어 있지 않아요." });
    return;
  }
...
3. reflection_point(감동포인트/성찰): 이 놀이를 마친 뒤 아이들 마음속에 남을 정서적 여운이나 깨달음을 한 문장으로. 디브리핑(성찰) 질문이 있다면 포함해도 좋음.
...
7. 반드시 아래 JSON 형식으로만 응답. 다른 설명, 코드블록 표시 없이 JSON만 출력.

{
  ...
  try {
    ...
    if (!textBlock) throw new Error("모델 응답이 비어있습니다.");
    ...
    res.status(500).json({ error: "생성에 실패했습니다.", detail: String(e) });
  }
};
